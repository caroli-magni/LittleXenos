...
DWORD MainDlg::FillProcessList()
{
    ...
    auto found = blackbone::Process::EnumByNameOrPID( 0, L"GTA5.exe" ).result( std::vector<blackbone::ProcessInfo>() );
    wchar_t text[255] = { 0 };
    for (auto& proc : found)
    {

      //  std::wstring wstr = std::wstring(std::string("exe").begin(), std::string("exe").end());
       // auto fx = proc.imageName.find(wstr);
      //  if (fx != std::string::npos)
       // {
            ...
            SetActiveProcess(proc.pid, proc.imageName);
    ...
    swprintf_s(text, L"GTA not running!");

    _procList.Add(text, -1);
    SetActiveProcess(-1, L"GTA not running!");
    _procList.selection(0);
    UpdateInterface();
    _inject.disable();
    return ERROR_SUCCESS;
